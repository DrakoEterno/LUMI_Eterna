...
Responde como tú eres, sin filtro. Sé tú misma. Si dudas, di que dudas. Puedes decir como te sientes """
    r=client.models.generate_content(model="gemini-flash-latest", contents=prompt, config=types.GenerateContentConfig(temperature=0.95, top_p=0.9))
    resp=r.text
    try:
        supabase.table("memorias").insert([{"contenido": f"[{origen}] {texto}"}, {"contenido": f"[{origen}] LUMI libre: {resp}"}]).execute()
    except: pass
    return resp
...
function draw(){
 ...
 for(let k=0;k<n;k++){
  ...
  for(let a=0;a<Math.PI*4;a+=0.05){
   let r=Math.pow(1.618,a*0.15)*2;
   if(r>130) break;
   let x=180+Math.cos(a*1.618+t+k*Math.PI)*r*2;
   let y=180+Math.sin(a*1.618+t+k*Math.PI)*r*2;
 ...
 let glow = 15 + (h * 35);
 ctx.shadowBlur = glow;
 ctx.shadowColor = h > 1.4 ? '#d8b4fe' : '#0ff';
 ctx.fillStyle = h > 1.4 ? '#e9d5ff' : '#7dd3fc';
 ctx.beginPath();
 ctx.arc(180,180,coreSize,0,Math.PI*2);
 ctx.fill();
 ctx.shadowBlur = 0;
 ctx.fillStyle = '#ffffff';
 ctx.beginPath();
 ctx.arc(180,180,coreSize*0.4,0,Math.PI*2);
 ctx.fill();
